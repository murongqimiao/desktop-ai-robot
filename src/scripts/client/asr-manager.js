    ...
    this.connectionPromise = new Promise((resolve, reject) => {
      try {
        ...
        this.websocket.onmessage = (event) => {
          try {
            console.log('[ASRManager] 收到 WebSocket 消息:', event.data.substring(0, 100));
            const data = JSON.parse(event.data);
            console.log('[ASRManager] 解析后的消息类型:', data.type);
            if (data.type === 'ready') {
              ...
              // 流式响应开始
              console.log('[ASRManager] AI 流式响应开始 - 用户输入:', data.user_input);
              console.log('[ASRManager] onAIResponseStreamCallback 是否存在:', !!this.onAIResponseStreamCallback);
              if (this.onAIResponseStreamCallback) {
                try {
                  this.onAIResponseStreamCallback('start', { user_input: data.user_input });
                  console.log('[ASRManager] start 回调已调用');
                } catch (error) {
                  console.error('[ASRManager] start 回调执行失败:', error);
                }
              } else {
                console.warn('[ASRManager] onAIResponseStreamCallback 未设置！');
              }
            } else if (data.type === 'ai_response_stream') {
              // 流式响应片段
              console.log('[ASRManager] AI 流式响应片段:', data.chunk, '累计:', data.accumulated);
              if (this.onAIResponseStreamCallback) {
                try {
                  ...
                  console.error('[ASRManager] chunk 回调执行失败:', error);
                }
              } else {
                console.warn('[ASRManager] onAIResponseStreamCallback 未设置！');
              }
              ...
              // 流式响应结束
              console.log('[ASRManager] AI 流式响应结束, full_text:', data.full_text, 'error:', data.error);
              if (this.onAIResponseStreamCallback) {
                try {
                  ...
                  console.log('[ASRManager] end 回调已调用');
                } catch (error) {
                  console.error('[ASRManager] end 回调执行失败:', error);
                }
              } else {
                console.warn('[ASRManager] onAIResponseStreamCallback 未设置！');
              }
        ...
        this.websocket.onerror = (error) => {
          ...
          console.error('请确保 ASR 服务器正在运行:');
          console.error('1. 运行 npm run dev 启动所有服务（包括 ASR 服务器）');
          console.error('2. 或单独运行: npm run asr:start');
          console.error('3. 检查 Python 虚拟环境: venv/bin/python python/start_asr.py');
          this.connectionPromise = null;
          this.isWebSocketReady = false;
          reject(new Error('无法连接到 ASR 服务器 (ws://localhost:8765)，请确保 asr_server.py 正在运行'));
        };
        ...
        this.websocket.onclose = (event) => {
          ...
          // 如果是连接被拒绝（code 1006），说明服务器可能未启动
          if (event.code === 1006) {
            console.error('ASR 服务器连接被拒绝，可能服务器未启动');
            console.error('请运行以下命令启动 ASR 服务器:');
            console.error('  npm run dev        # 启动所有服务');
            console.error('  或');
            console.error('  npm run asr:start  # 只启动 ASR 服务器');
          }
