...
export class HearController {
  ...
  async init() {
    ...
    try {
      ...
      if (this.aiResponseStreamCallback) {
        console.log('[HearController] init() 时设置已保存的 AI 流式响应回调');
        this.asrManager.onAIResponseStream(this.aiResponseStreamCallback);
      } else {
        console.log('[HearController] init() 时没有已保存的 AI 流式响应回调');
      }
  ...
  setAIResponseStreamCallback(callback) {
    console.log('[HearController] 设置 AI 流式响应回调', callback);
    // 保存回调，即使 asrManager 还没初始化
    this.aiResponseStreamCallback = callback;
    
    // 如果 asrManager 已经初始化，立即设置
    if (this.asrManager) {
      console.log('[HearController] asrManager 已初始化，立即设置回调');
      this.asrManager.onAIResponseStream(callback);
    } else {
      console.log('[HearController] asrManager 未初始化，回调已保存，将在 init() 时设置');
    }
  }
